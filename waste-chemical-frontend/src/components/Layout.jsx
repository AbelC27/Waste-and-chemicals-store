...
import { LogOut, Home, Trash2, Beaker, Menu, X, Moon, Sun, BarChart3, Download, Bell, BookUser, Shield } from 'lucide-react'
...
const navItems = [
  ...
  { path: '/user-management', icon: Shield, label: 'User Mgmt', permission: 'admin:manage_users' },
]

const Sidebar = ({ sidebarOpen, setSidebarOpen, notificationCount, onBellClick }) => {
  // FIX: Get user, profile, AND signOut directly from the useAuth hook at the top level.
  ...
  // FIX: This function now correctly uses the signOut function obtained from the hook above.
    ...
    <aside className={`fixed inset-y-0 left-0 z-50 w-64 bg-white dark:bg-gray-800 shadow-xl transform transition-transform duration-300 ease-in-out lg:translate-x-0 lg:static lg:inset-0 ${sidebarOpen ? 'translate-x-0' : '-translate-x-full'}`}>
      <div className="flex flex-col h-full">
        <div className="flex items-center justify-between h-16 px-6 bg-indigo-600 dark:bg-indigo-700">
          ...
          <button onClick={() => setSidebarOpen(false)} className="lg:hidden text-white hover:text-indigo-200"><X className="w-6 h-6" /></button>
        </div>
        <nav className="flex-1 px-4 py-6 space-y-2">
          {navItems.map((item) => {
            ...
            return (
              <Link key={item.path} to={item.path} onClick={() => setSidebarOpen(false)} className={`group flex items-center px-3 py-2.5 text-sm font-medium rounded-lg transition-all duration-200 ${isActive ? 'bg-indigo-50 dark:bg-indigo-900/50 text-indigo-700 dark:text-indigo-300' : 'text-gray-700 dark:text-gray-300 hover:bg-gray-100 dark:hover:bg-gray-700 hover:text-gray-900 dark:hover:text-white'}`}>
        ...
        <div className="px-4 py-4 border-t border-gray-200 dark:border-gray-700">
          ...
          <div className="flex items-center p-3 bg-gray-50 dark:bg-gray-700 rounded-lg">
            <div className="w-8 h-8 bg-indigo-500 rounded-full flex items-center justify-center"><span className="text-sm font-medium text-white">{user?.email?.charAt(0).toUpperCase()}</span></div>
            <div className="ml-3 flex-1 min-w-0">
              <p className="text-sm font-medium text-gray-900 dark:text-white truncate">{user?.email}</p>
              <p className="text-xs text-gray-500 dark:text-gray-400">{userProfile?.roles?.name || 'Loading...'}</p>
            </div>
            <button onClick={handleSignOut} className="ml-2 p-1 text-gray-400 hover:text-red-500 transition-colors" title="Sign Out"><LogOut className="w-4 h-4" /></button>
          </div>
        </div>
      </div>
    </aside>
...
const Layout = ({ children }) => {
  ...
  useEffect(() => {
    api.get('/api/notifications').then(res => {
      if (res.data) setNotificationCount(res.data.length)
        ...
        <div className="flex-1 flex flex-col overflow-hidden">
          <header className="bg-white dark:bg-gray-800 shadow-sm border-b border-gray-200 dark:border-gray-700">
            <div className="flex items-center justify-between h-16 px-6">
              <div className="flex items-center">
                <button onClick={() => setSidebarOpen(true)} className="p-2 text-gray-500 hover:text-gray-700 dark:text-gray-400 dark:hover:text-gray-200 rounded-lg lg:hidden"><Menu className="w-6 h-6" /></button>
